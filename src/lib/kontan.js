module.exports = function(fn) {
  this.validate(fn);
  axios.get(`https://www.kontan.co.id/`, config)
  .then((response) => {
    let hasil = [];
    ...
    $("div.news-list > ul > li").each(function(i, elem) {
      ...
      let thumbnail = $(this).find("div.image-thumb").find("img").attr("data-src");
      let category = $(this).find("a.link-orange").text().trim();
      let upload_date = $(this).find("div.box-news.fleft").find(".meta.fs12").text().split(category + " | ")[1].trim();
      const result = {
        title,
        news_url,
        thumbnail,
